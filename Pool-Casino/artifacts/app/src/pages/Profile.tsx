...
import { getUserStats, claimDailyReward, DailyRewardResponse } from '@workspace/api-client-react';
...
export default function Profile() {
  ...
  const claimMutation = useMutation<DailyRewardResponse, Error, void>({
        ...
        <div className="card text-center">
          ...
          <p className="text-2xl font-bold text-white">
            {(stats?.gamesPlayed ?? 0) > 0 
              ? Math.round(((stats?.totalWins ?? 0) / (stats?.gamesPlayed ?? 1)) * 100)
              : 0
            }%
          </p>
          <p className="text-sm text-gray-400">
